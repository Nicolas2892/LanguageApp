...
import VerbConfigureError from '../error'

describe('verbs/configure error.tsx', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })
  ...
  it('calls Sentry.captureException', () => {
    ...
    render(<VerbConfigureError error={err} reset={() => {}} />)
  ...
  it('has a Reintentar button that calls reset', async () => {
    ...
    const reset = vi.fn()
    render(<VerbConfigureError error={new Error('test')} reset={reset} />)
