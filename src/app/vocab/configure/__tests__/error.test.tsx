...
import VocabConfigureError from '../error'

describe('vocab/configure error.tsx', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })

  it('renders branded fallback UI', () => {
    render(<VocabConfigureError error={new Error('test')} reset={() => {}} />)
    expect(screen.getByText('Algo salió mal')).toBeInTheDocument()
  })

  it('calls Sentry.captureException', () => {
    const err = new Error('config crash')
    render(<VocabConfigureError error={err} reset={() => {}} />)
  ...
  it('has a Reintentar button that calls reset', async () => {
    ...
    const reset = vi.fn()
    render(<VocabConfigureError error={new Error('test')} reset={reset} />)
