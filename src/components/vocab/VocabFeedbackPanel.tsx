...
export function VocabFeedbackPanel({ result, onNext, onTryAgain, isLast, answerVariants, completedSentence }: Props) {
  const { outcome, userAnswer, correctForm } = result
  ...
  return (
    <div className="senda-feedback-card space-y-4">
      ...
      <div className="space-y-2 text-sm">
        ...
        {answerVariants && answerVariants.length > 0 && (
          <p className="text-xs text-[var(--d5-muted)]">
            También válido: {answerVariants.join(', ')}
          </p>
        )}
      </div>
