...
import type { VocabCategoryStat } from '@/lib/vocab/types'

interface Props {
  correct: number
  total: number
  categoryStats: VocabCategoryStat[]
  onPracticeAgain: () => void
}

export function VocabSummary({ correct, total, categoryStats, onPracticeAgain }: Props) {
  ...
  // Sort category stats worst first
    ...
    <div className="space-y-6 text-center py-8">
      ...
      {sorted.length > 0 && (
        <section className="senda-card text-left space-y-3">
          <p className="senda-eyebrow">Por categoría</p>
          <div className="space-y-2">
            {sorted.map((s) => {
              const catPct = s.total > 0 ? Math.round((s.correct / s.total) * 100) : 0
              const label = CATEGORY_LABELS[s.category as VocabCategory] ?? s.category
              return (
      ...
      <div className="flex flex-col items-center gap-3">
        ...
        <Link href="/verbs?tab=vocab" className="senda-cta-outline">
          Ver Léxico
        </Link>
      </div>
    </div>
