...
describe('SprintCard', () => {
  ...
  it('renders collapsed by default showing Configure button', () => {
    render(<SprintCard dueCount={5} modules={mockModules} />)
    expect(screen.getByRole('button', { name: /configure sprint/i })).toBeTruthy()
  ...
  it('expands when Configure button is clicked', () => {
    render(<SprintCard dueCount={5} modules={mockModules} />)
    fireEvent.click(screen.getByRole('button', { name: /configure sprint/i }))
    expect(screen.getByRole('button', { name: 'Start Sprint →' })).toBeTruthy()
  })

  it('shows "All" module chip selected by default', () => {
    render(<SprintCard dueCount={5} modules={mockModules} />)
    fireEvent.click(screen.getByRole('button', { name: /configure sprint/i }))
    const allBtn = screen.getByRole('button', { name: 'All' })
    expect(allBtn.className).toContain('bg-orange-100')
  })
  ...
  it('selecting a module chip activates it and deactivates All', () => {
    render(<SprintCard dueCount={5} modules={mockModules} />)
    fireEvent.click(screen.getByRole('button', { name: /configure sprint/i }))
    ...
    expect(modBtn.className).toContain('bg-orange-100')
    const allBtn = screen.getByRole('button', { name: 'All' })
    expect(allBtn.className).not.toContain('bg-orange-100')
  })
  ...
  it('10 min chip is selected by default', () => {
    render(<SprintCard dueCount={5} modules={mockModules} />)
    fireEvent.click(screen.getByRole('button', { name: /configure sprint/i }))
